...
*Expectation maximisation (EM) algorithm for parameter estimation.* Baum-Welch Learning is in contrast to Viterbi Learning, as it does do *soft* assignments of states in the Expectation step. Our goal here is to compute $P(\pi_i = k \vert x)$, the responsibility of state $k$ at the $i$-th timestep. To do this, we first compute $P(\pi_i = k, x)$ : 
